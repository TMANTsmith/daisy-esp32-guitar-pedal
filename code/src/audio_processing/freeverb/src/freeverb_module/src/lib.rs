        ...
        match command {
            Command::SetParameter(id, value) => match Parameters::from_usize(id).unwrap() {
                Parameters::Dampening => {
                    self.freeverb.set_dampening(value as f32);
                }
                Parameters::Width => {
                    self.freeverb.set_width(value as f32);
                }
                Parameters::RoomSize => {
                    self.freeverb.set_room_size(value as f32);
                }
                Parameters::Freeze => {
                    self.freeverb.set_freeze(value != 0.0);
                }
                Parameters::Dry => {
                    self.freeverb.set_dry(value as f32);
                }
                Parameters::Wet => {
                    self.freeverb.set_wet(value as f32);
                }
            },
        }
    }
...
impl AudioProcessor for FreeverbProcessor {
    fn process_stereo(&mut self, input: &[f32], output: &mut [f32]) {
        ...
        for i in (0..input.len()).step_by(2) {
            let result = self.freeverb.tick((input[i] as f32, input[i + 1] as f32));
